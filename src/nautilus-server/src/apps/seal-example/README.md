...
`--id`: The enclave object ID (or identifier derived from it). This should match the `id` used in Step 2. Use `xx` as shown, or the actual enclave object ID.
...
`-t`: Threshold used for encryption (must be ≤ number of key servers).
...
In this call, the enclave creates a certificate and constructs a PTB calling `seal_approve` with the enclave object ID. The enclave wallet signs the enclave's public key to commit to it. A session key signs the request and returns the encoded FetchKeyRequest. The `id` parameter should match the ID used in step 1.

```bash
curl -X POST http://localhost:3001/admin/init_seal_key_load -H 'Content-Type: application/json' -d '{"enclave_object_id": "<ENCLAVE_OBJECT_ID>", "initial_shared_version": <ENCLAVE_OBJ_VERSION>, "id": "1"}'
...
This step is done in the host that the enclave runs in, that can communicate to the enclave via 3001. The enclave decrypts and caches ALL keys from ALL seal responses in memory. No encrypted objects are decrypted at this step - the keys are simply cached for later on-demand decryption.
...
This step demonstrates the on-demand decryption pattern. The enclave uses the cached keys from Step 4 to decrypt the encrypted weather API key object from Step 1. This endpoint can be called multiple times with different encrypted objects without needing to re-fetch keys from Seal servers.
...
Replace `<ENCRYPTED_OBJECT>` with the output from Step 1.
...
The cache-then-decrypt workflow makes it efficient to handle multiple secrets. All secrets encrypted with the same enclave ID can be decrypted after caching keys once.

**Step 1: Encrypt multiple secrets using the same enclave ID**
```bash
# Encrypt weather API key (use same --id for all secrets)
cargo run --bin seal-cli encrypt --secret <WEATHER_API_KEY_HEX> --id <ENCLAVE_OBJ_ID> -p $APP_PACKAGE_ID -t 2 -k ... -n testnet
Encrypted object:
<ENCRYPTED_WEATHER_OBJECT>

# Encrypt database password (use same --id)
cargo run --bin seal-cli encrypt --secret <DB_PASSWORD_HEX> --id <ENCLAVE_OBJ_ID> -p $APP_PACKAGE_ID -t 2 -k ... -n testnet
Encrypted object:
<ENCRYPTED_DB_OBJECT>
```

**Steps 2-4: Cache keys once**
Run steps 2-4 once to cache the Seal keys for the enclave ID.

**Step 5: Decrypt any encrypted object on-demand**
Once keys are cached, decrypt any encrypted object:
```bash
# Decrypt weather API key
curl -X POST http://localhost:3001/admin/provision_weather_api_key -d '{"encrypted_object": "<ENCRYPTED_WEATHER_OBJECT>"}'

# Decrypt other secrets using custom endpoints as needed
```

The key benefit: Encrypt unlimited secrets with the same enclave ID, cache keys once, decrypt on-demand without re-fetching from Seal servers. 
