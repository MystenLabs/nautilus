                ...
                if let Some(object_data) = response.data {
                    if let Some(content) = object_data.content {
                        if let SuiParsedData::MoveObject(parsed_data) = content {
                            ...
                            // todo: maybe fetch pk and compare to config. or not put in config.
...
#[command(about = "Utility CLI for Seal: encrypt and fetch keys", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
    ...
    match cli.command {
        Commands::Encrypt {
            ...
            let key_server_ids = config
                .key_servers
                .iter()
                .map(|s| NewObjectID::from_str(s).map_err(|e| format!("Invalid object ID: {}", e)))
